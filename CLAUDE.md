...
### Next (in order) ⏳
0. **Repair the Whisper cache.** Real-rig `dry_run.ps1` history: the first run (2026-09-04) was 7/3 with `gemma4:e2b` missing; after pulling it, 9/1 with only step 7 failing — `huggingface_hub.LocalEntryNotFoundError` from inside `WhisperModel()`. So Ollama and Gemma 4 are fine and the large-v3 cache at `D:\hf-cache` is incomplete (or `hf-mirror.com` was unreachable). `python dry_run_smoke.py whisper` says which. Re-download with `python -c "from faster_whisper import WhisperModel; WhisperModel('large-v3')"` — it resumes.
   Confirmed working on the rig across those runs: the console UTF-8 fix (Chinese now renders), and the B1 gate thresholds on both the Chinese and the English probe sentences.
