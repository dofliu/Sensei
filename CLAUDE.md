...
### Next (in order) ⏳
0. **Re-run `.\dry_run.ps1`.** Real-rig history: run 1 (2026-09-04) 7/3, `gemma4:e2b` not pulled; run 2 9/1, step 7 dying in `WhisperModel()` with `huggingface_hub.LocalEntryNotFoundError`; run 3 the same, with the cache proven complete (`model.bin` 3087 MB) *and* `refs/main` matching the snapshot name.
   The remaining suspect is the **bytes** of `refs/main`. huggingface_hub reads that file with a plain `f.read()` and joins the result straight onto `snapshots/`, so a trailing `\n`, a CRLF, a BOM or a leading space makes a byte-perfect cache unresolvable — and every tool that `.strip()`s, including two earlier versions of this preflight, shows it as correct. Reproduced in the sandbox: each of those four patterns raises exactly the rig's error on an otherwise working cache. `dry_run_smoke.py whisper` now prints the raw bytes and, when they are recoverable, the byte-exact repair.
   Confirmed working on the rig: the console UTF-8 fix (Chinese renders), Ollama + Gemma 4 (step 6), and the B1 gate thresholds on the Chinese and English probe sentences.
