...
### Next (in order) ⏳
**Preflight is green.** `.\dry_run.ps1` reached 10/10 on the rig on 2026-09-04 — env vars, Whisper cache, Ollama + `gemma4:e2b`, deps, mic, the LLM and pipeline smokes, and the B1 gate / session / handout step. Two environment bugs were found and fixed getting there: `HF_HUB_CACHE` shadowing `HF_HOME` (see §3), and `localhost` resolving to IPv6 `[::1]` in PowerShell 5.1 while Ollama listens only on IPv4, which made `start_sensei.ps1` declare a running daemon dead and then fail to start a second one on the occupied port. The launcher now uses `127.0.0.1` and falls back to `ollama list` when the REST probe is blocked.

1. **Teach one real lecture.** `.\start_sensei.ps1`, name the course, start continuous listening, teach, export the handout. This is the only thing that has never been done.
2. **The queue is dropping.** First real lecture (2026-09-04): card volume felt right, the `dropped` counter is non-zero, and whether `no_card` refuses things it should keep is still unknown. A drop means an utterance was thrown away before ASR — the teacher said it and nothing anywhere records what. Every utterance now carries `_audio_s` / `_queue_ms` / `_asr_ms` / `_llm_ms`, and drops are written to `skipped.jsonl` as `gate="dropped"`, so the next lecture answers which of ASR or the LLM is eating the budget before any constant is touched. The likely first lever is `GATE_MIN_CONTENT`: the rule layer is the only path that costs no LLM call at all, and in a lecture most utterances should be skipped.
3. **Tune the B1 constants** from that lecture. They are defaults, not measurements — all of them are named constants at the top of `core/live_mic.py` and `core/pipeline.py`, and `python -m bench.segmenter_probe` shows what a change does. The three questions to answer: too many cards or too few, do cards arrive late (the `dropped` counter), does the skip log show `no_card` refusing things it should have kept.
4. **C2 Evaluation bench** — `bench/utterances.jsonl`, template hit-rate and gate accuracy for e2b vs e4b (paper material). The `_gate` field and `skipped.jsonl` from a real lecture are the raw data.
5. C1 student-side quiz answering over LAN — only after checking the classroom Wi-Fi lets phones reach the laptop.
...
| **`ASRConfig.DOWNLOAD_ROOT` pins the Whisper cache; `WhisperModel(download_root=...)`** | `HF_HUB_CACHE` takes precedence over `HF_HOME` inside huggingface_hub. The rig had `HF_HUB_CACHE=D:\AI_Cache\HuggingFace\hub` set for an unrelated project, so faster-whisper looked there, found an incomplete cache, and raised `LocalEntryNotFoundError` while `D:\hf-cache` held a perfect 3.1 GB copy. Sensei must not depend on which other projects share the machine. `dry_run_smoke.py whisper` reads this same constant instead of re-deriving a path, and prints every HF cache variable plus a note when `HF_HUB_CACHE` disagrees. | Don't drop `download_root` / `cache_dir`. Don't let the preflight derive the cache path independently of `ASRConfig` — a check that agrees with the app only by coincidence is what turned this into a four-round hunt. |
| **Whisper loads `local_files_only=True` first** | faster-whisper otherwise asks the Hub which commit `main` points at on *every* startup, so a fully cached laptop still fails to start with no network — which contradicts §1A point 4. Verified in the sandbox: two identical caches, one with `refs/main` and one without, resolve and raise `LocalEntryNotFoundError` respectively. Only cache-miss/offline errors (both `OSError` subclasses) fall through to a networked retry, so a CUDA failure is not retried slowly. | Don't drop the offline-first attempt to "simplify" the loader. Don't widen the `except` past `OSError`. |
...
Environment variables (User scope):
  ...
  HF_HUB_CACHE  = D:\AI_Cache\HuggingFace\hub   <- belongs to another project;
                  huggingface_hub prefers it over HF_HOME, which is why Sensei
                  pins ASRConfig.DOWNLOAD_ROOT instead of trusting the environment
