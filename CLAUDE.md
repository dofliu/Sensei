...
### Next (in order) ⏳
0. **Re-run `.\dry_run.ps1`.** Real-rig history: run 1 (2026-09-04) was 7/3 with `gemma4:e2b` missing; run 2 was 9/1, only step 7 failing with `huggingface_hub.LocalEntryNotFoundError` from inside `WhisperModel()`. The cache turned out to be **complete** (`model.bin` 3087 MB, tokenizer, vocabulary all present) — the failure was the Hub round-trip faster-whisper does on every startup to resolve which commit `main` is. `core/asr.py` now loads `local_files_only=True` first, so that round-trip is gone; `dry_run_smoke.py whisper` checks `refs/main` and runs the real offline resolution.
