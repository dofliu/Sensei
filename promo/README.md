...
> **⚠️ 已交付的 mp4 停在 2026-09-02,B1 / B3 之前。**
> `build_scenes.py` 與 `scene*.html` 已經更新(scene06 補上第 8 個工具 `no_card`、
> scene19 的「學生講義」改成實際的 `handout.html`),但影片本身還沒重新算過。
> 要重新出片就照下面「怎麼改」的流程跑一次;總長仍然是 179.4 秒。
> 另外**連續聆聽(B1)與課堂 session(B3)目前全片都還沒有專門的一景** ——
> 如果之後要當作對外的介紹片,這兩個現在是最大的賣點,值得各加一景。
