# Sensei · Codex Project Context

> **For Codex (and any future AI assistant working on this project): read this entire file before making any changes.** It captures decisions, constraints, and the reasoning behind them. Reverting them without understanding why will hurt the project.
...
### Not yet started ⏳
- Domain glossary expansion (currently only auto-control terms) — soft, iterative
- Add real-app screenshots (agent_skill key_fact, SG90 key_fact, paper-editorial enumeration / quiz / `/display`) to `docs/screenshots/`
- WRITEUP.md final polish pass before Kaggle submission

### Done since project start
- ~~Theme switching (dark / light / paper)~~ — shipped 2026-05-09
- ~~SWOT / pyramid templates~~ — shipped 2026-05-09; ~~quiz_card with spoken-trigger guard~~ — shipped 2026-05-11; ~~key_fact (single-concept) template~~ — shipped 2026-05-12; ~~fishbone~~ dropped 2026-05-09
- ~~Native Gemma 4 function-calling~~ — tools path is primary, JSON mode is silent fallback
- ~~Real classroom audio testing pass~~ — done 2026-05-11 via live mic
- ~~Demo video~~ — submitted 2026-05-14 as 1:45 motion-graphics presentation (the originally planned three-track classroom shoot was deferred for environment reasons; see DEMO_SCRIPT.md)
...
- [x] **Demo video** (~1:45) — submitted as a motion-graphics presentation: cold-open + why-not-cloud-LLM + on-device terminal-capture proof (`ollama run gemma4:e2b`) + seven-template tour + outro. The originally planned three-track classroom shoot at NCUT (see `DEMO_SCRIPT.md`) was deferred for environment reasons; the repo's real-app screenshots and `dry_run.ps1` carry the technical-credibility load instead.
...
Day 7  · 2026-05-14 done   ✅ Demo video shipped as 1:45 motion-graphics presentation (AI-rendered visuals + real `ollama run gemma4:e2b` terminal capture); classroom-shoot plan in DEMO_SCRIPT.md preserved for future re-shoot
Day 8  · 2026-05-16 (Sat)  ⏳ Final WRITEUP.md pass + add real-app screenshots to docs/screenshots/
Day 9  · 2026-05-17 (Sun)  ⏳ Buffer / README polish / writeup final pass
Day 10 · 2026-05-18 (Mon)  📤 SUBMIT before 23:59 UTC
```

**Net status as of 2026-05-14**: code complete; 8 templates shipped; demo video shipped as motion-graphics presentation; quiz spoken-trigger guard and key_fact template both landed mid-stream. Remaining work is repo-side polish (screenshots, WRITEUP final pass) and the Kaggle submission click before 2026-05-18 23:59 UTC.

---

## 8. Immediate next tasks (final-stretch order)

Build complete. Video shipped. Remaining queue is repo-side technical-credibility scaffolding:

1. **Add real-app screenshots to `docs/screenshots/`** — agent_skill key_fact (120 px highlight), SG90 key_fact, paper-editorial enumeration, comparison, quiz_card with 2×2 options, `/display` route. These are the proof artifacts the submission video doesn't show (the video uses stylized renders + one terminal capture; repo carries the actual paper-editorial rendering evidence). README §Screenshots already references them.

2. **WRITEUP.md final pass** — template count and feature list already current; only polish needed before submission.

3. **Kaggle submission form** — paste GitHub URL, video URL, WRITEUP.md text, license attestation. Final click before 2026-05-18 23:59 UTC.

Resolved earlier:
- 2026-05-11: Day 4 real-classroom audio testing pass; quiz_card + spoken-trigger guard shipped; DEMO_SCRIPT Demo C swapped flow → quiz
- 2026-05-12: key_fact template shipped (8th template) for single-concept speech patterns
- 2026-05-14: Submission video v2 finalized (motion-graphics path, with model name corrected to `gemma4:e2b` and terminal-capture proof beat)
